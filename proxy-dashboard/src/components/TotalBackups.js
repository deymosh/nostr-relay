...
export default function TotalBackups({
  loading,
  events,
  deletionEvents,
  supportedEventKinds,
  ...
  if (!loading) {
    ...
    events.forEach((event) => {
      // with filters applied, all kinds should be supported
  ...
  const totalDeletionEvents = deletionEvents;

  const totalDeletionEventsAnimatedCount = useMotionValue(totalDeletionEvents);
  const totalDeletionEventsAnimatedCountRounded = useTransform(
    totalDeletionEventsAnimatedCount,
    (latestValue) => Math.round(latestValue),
  );
  ...
  useEffect(() => {
    if (!loading) {
      const controlEvents = animate(totalEventsAnimatedCount, totalEvents);
  ...
  useEffect(() => {
    if (!loading) {
      const controlDeletionEvents = animate(
        totalDeletionEventsAnimatedCount,
        totalDeletionEvents,
      );

      return () => {
        controlDeletionEvents.stop();
      };
    }
  }, [totalDeletionEvents, loading]);
    ...
    <div>
      <p className="text-slate-700 px-10 dark:text-slate-300 leading-relaxed mt-2">
        <motion.span className="text-8xl md:text-7xl lg:text-9xl font-semibold bg-clip-text text-transparent bg-gradient-to-r from-pink-500 to-violet-500">
          {totalEvents > 5000 ? "5000+ (max)" : totalEventsAnimatedCountRounded}
        </motion.span>{" "}
      </p>
      {totalEvents ? (
      ...
      <p className="text-slate-700 px-10 dark:text-slate-300 leading-relaxed mt-2 text-right">
        {"🗑 Deleted Actions: "}
        <motion.span className="text-2xl md:text-2xl lg:text-2xl font-semibold bg-clip-text text-transparent bg-gradient-to-r from-pink-500 to-violet-500">
          {totalDeletionEventsAnimatedCountRounded}
        </motion.span>
      </p>
    </div>
  );
